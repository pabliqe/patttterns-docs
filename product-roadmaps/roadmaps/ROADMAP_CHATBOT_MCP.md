...
- `netlify/functions/chatbot-proxy.mts` — Gemini proxy (serverless, streaming SSE)
...
**Implementation:** Edge function should read from a private Notion cache path (non-public build artifact) generated by `build:content`.

**Files to update:**
- `netlify/edge-functions/mcp.ts` — add `resources/list` and `resources/read` methods

**Effort:** ~1 day  
**Dependencies:** private Notion cache populated (planned migration from public path)
