...
### 3) public/pattern-metadata-cache.json

Generated by: `scripts/build-pattern-metadata-cache.mjs`
...
- metadata enrichment from `public/pattern-metadata-cache.json` when metadata tooling is enabled
...
- `build-pattern-metadata-cache`: generate and optionally sync AI description intelligence
